...
def create_test_train(df):
    ...
    # Variable to add the user with the first instance of each book to the training dataset
    # otherwise the training and test datasets will have different dimensions.
    user_first_copy_book = set(df.groupby(by="id_book").first()["user"])

    train = df.query(
        "user in @train_user or user in @user_first_copy_book")
    test = df.query(
        "user in @test_user and user not in @user_first_copy_book")
    return train, test
